 ...
 * Version: 1.2.1
 * Ref Url: https://www.jqueryscript.net/form/Drop-Down-Combo-Tree.html
 */
...
;(function ($, window, document, undefined) {
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        var itemHtml = "",
            isThereSubs = sourceItem.hasOwnProperty("sub_categories");
        var selectionDisabled = "";
        if (this.options.disableParent)
            selectionDisabled = (isThereSubs) ? "disabled" : "";
        else if (this.options.disableSpecificItem)
            selectionDisabled = (sourceItem.disable_me) ? "disabled" : "";
        ...
        if (this.options.disableSpecificItem && selectionDisabled.length > 0)
            itemHtml += '<span data-skin="dark" data-toggle="kt-tooltip" data-original-title="Not selectable">';

        if (this.options.isMultiple)
            itemHtml += '<span data-id="' + sourceItem.id + '" class="comboTreeItemTitle"' + selectionDisabled + '><input type="checkbox"' + selectionDisabled + '>' + sourceItem.category + '</span>';
        else
            itemHtml += '<span data-id="' + sourceItem.id + '" class="comboTreeItemTitle"' + selectionDisabled + '>' + sourceItem.category + '</span>';

        if (this.options.disableSpecificItem && selectionDisabled.length > 0)
            itemHtml += '</span>';

        if (isThereSubs)
            itemHtml += this.createSourceSubItemsHTML(sourceItem.sub_categories, sourceItem.id);
    ...
    ComboTree.prototype.singleItemClick = function (ctItem) {
        $flag = true;
        if (this.options.disableParent) {
            $flag = ($(ctItem).parent('li').hasClass('ComboTreeItemParent'));
        } else if (this.options.disableSpecificItem) {
            $flag = ($(ctItem).attr("disabled") == "disabled");
        }
    ...
    ComboTree.prototype.multiItemClick = function (ctItem) {
        if (this.options.cascadeSelect) {
            var isParent = $(ctItem).closest('li').hasClass('ComboTreeItemParent');
            var isDisabledItem = ($(ctItem).attr("disabled") == "disabled");
            if (isParent && !this.options.disableParent && !isDisabledItem) {
    ...
    ComboTree.prototype.isItemInArray = function (item, arr) {
        for (var i = 0; i < arr.length; i++) {
            if (item.id == arr[i].id && item.category == arr[i].category)
                return i + "";

            if (arr[i].hasOwnProperty("sub_categories")) {
                let found = this.isItemInArray(item, arr[i].sub_categories);
                if (found)
                    return found;
            }
        }
    ...
    ComboTree.prototype.refreshInputVal = function () {
        ...
        if (this.options.isMultiple) {
            for (var i = 0; i < this._selectedItems.length; i++) {
                tmpTitle += this._selectedItems[i].category;
                if (i < this._selectedItems.length - 1)
                    tmpTitle += ", ";
            }
        }
        else {
            tmpTitle = this._selectedItem.category;
        }
        ...
        if (itemId && source) {
            for (let i = 0; i < source.length; i++) {
                if (source[i].id == itemId)
                    return {id: source[i].id, category: source[i].category};
                if (source[i].hasOwnProperty("sub_categories")) {
                    let found = this.findItembyId(itemId, source[i].sub_categories);
                    if (found)
                        return found;
                }
            }
    ...
    ComboTree.prototype.getSelectedNames = function () {
        if (this.options.isMultiple && this._selectedItems.length > 0) {
            var tmpArr = [];
            for (i = 0; i < this._selectedItems.length; i++)
                tmpArr.push(this._selectedItems[i].category);

            return tmpArr;
        }
        else if (!this.options.isMultiple && this._selectedItem.hasOwnProperty('id')) {
            return this._selectedItem.category;
        }
        return null;
    };
